...
st.info("💡 **Catatan:** Area ini disiapkan sebagai wadah (*skeleton*) untuk integrasi model Machine Learning yang sedang diuji coba di Google Colab.")
...
with tab1:
    ...
    with col_supp:
        min_supp = st.slider("Batas Kepopuleran (Support)", min_value=0.001, max_value=0.1, value=0.01, step=0.001, format="%.3f", help="Batas minimal seberapa sering kombinasi produk muncul di seluruh transaksi. Semakin kecil, semakin banyak kemungkinan barang langka yang tergali.")
    with col_conf:
        min_conf = st.slider("Kekuatan Hubungan (Confidence)", min_value=0.1, max_value=1.0, value=0.2, step=0.05, format="%.2f", help="Peluang (dalam persentase) barang kedua ikut terbeli jika barang pertama dimasukkan ke keranjang.")

    if st.button("🚀 Jalankan Analisis Bundling", type="primary"):
            ...
            if "error" in result:
                ...
                if not rules_df.empty:
                    ...
                    **📖 Cara Membaca Score (Mengapa direkomendasikan?):**
                    ...
                    st.write("")
                    ai_ctx = f"""
[DATA HASIL MACHINE LEARNING: MARKET BASKET ANALYSIS (BUNDLING)]
Rekomendasi Utama: Bundling antara '{top_rule['antecedents']}' dan '{top_rule['consequents']}'.
Statistik: Support={top_rule['support']:.3f}, Confidence={top_rule['confidence']*100:.1f}%, Lift={top_rule['lift']:.2f}.
Tolong berikan strategi pemasaran lanjutan, ide nama promo, atau target market untuk bundle ini.
"""
                    st.button("🤖 Diskusikan Strategi Bundling Ini dengan AI Data Analyst", key="btn_ai_bundling", on_click=trigger_chatbox, args=(ai_ctx,))
                else:
    ...
    with st.spinner("Memproses seluruh data pelanggan dengan mesin inferensi..."):
        ...
        if df_segment.empty:
            ...
            **Insight Operasional (Data-Driven):**
            ...
            ai_ctx = f"""
[DATA HASIL SISTEM PAKAR: SEGMENTASI PELANGGAN (LOYALITAS)]
Terdapat {vip_count} pelanggan Super VIP (total belanja Rp {vip_rev:,.0f}) dan {loyal_count} pelanggan Loyal.

[KONTEKS BISNIS]
- Rata-rata omzet bulanan toko adalah Rp 300 Juta.
- Pelanggan 'Super VIP' adalah mereka yang menyumbang > 1% omzet (kemungkinan besar mitra B2B, agen, atau kafe, bukan pembeli eceran biasa).

[TARGET OPERASIONAL]
Pindahkan pelanggan VIP dan Loyal ini ke Direct Order (WhatsApp) agar toko tidak terkena pajak/potongan admin marketplace yang besar.
Tolong buatkan draf pesan WhatsApp (copywriting) yang personal, eksklusif, dan sopan untuk merayu para 'Sultan' ini agar mau order langsung via WA dengan iming-iming prioritas packing dan diskon agen/kafe.
"""
            st.button("🤖 Diskusikan Pelanggan VIP Ini dengan AI Data Analyst", key="btn_ai_clustering", on_click=trigger_chatbox, args=(ai_ctx,))
...
with tab3:
    ...
    with col_days:
        forecast_days = st.number_input("Rentang Waktu Prediksi (Hari)", min_value=7, max_value=90, value=30, step=7)
    with col_btn:
        st.write("") # spacing
        st.write("") # spacing
        run_forecast = st.button("📈 Jalankan Prediksi Omzet", type="primary")
        ...
        with st.spinner("Memproses data historis dan melatih model Holt-Winters ETS..."):
            ...
            if "error" in result:
                ...
                *(Catatan: Mengapa garis prediksi terlihat mendatar/stagnan? Karena sistem prediksi (Machine Learning Klasik) ini sangat berhati-hati. Jika ia melihat ada penurunan omzet di minggu-minggu terakhir (seperti akhir Juni), ia akan memproyeksikan tren yang stabil/datar untuk mencegah Anda kulakan barang terlalu banyak alias overstock).*
                ...
                st.write("")
                ai_ctx = f"""
[DATA HASIL MACHINE LEARNING: FORECASTING OMZET (Holt-Winters ETS)]
Total Omzet Prediksi {forecast_days} hari ke depan: Rp {metrics['total_forecast']:,.0f} (Rata-rata Rp {metrics['avg_daily_forecast']:,.0f}/hari).
Error Model (MAPE): {mape}%.
Insight Tambahan: Berdasarkan grafik masa lalu, sering terjadi lonjakan di akhir bulan/awal bulan. 
Tolong analisa mengapa tren ini terjadi (misalnya efek payday/gajian), bagaimana cara memanfaatkannya untuk pengadaan stok, dan apa strategi marketing yang tepat di tanggal-tanggal sibuk tersebut.
"""
                st.button("🤖 Diskusikan Grafik & Tren Omzet Ini dengan AI Data Analyst", key="btn_ai_forecasting", on_click=trigger_chatbox, args=(ai_ctx,))
